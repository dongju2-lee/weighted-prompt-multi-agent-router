...
```mermaid
flowchart TD
    A["🧑‍💻 사용자 질문<br/>음식 추천해줘"] --> B["🔤 질문 임베딩<br/>벡터 변환"]
    B --> C["🗄️ 벡터 데이터베이스<br/>유사 질문 검색"]
    C --> D["📊 패턴 분석<br/>에이전트별 비율 계산"]
    D --> E["⚖️ 가중치 적용<br/>비율 조정"]
    E --> F["📝 향상된 프롬프트<br/>과거 데이터 + 비율"]
    F --> G["🤖 슈퍼바이저 에이전트<br/>최종 라우팅 결정"]
